...
class YouTubeTranscriberServer {
  ...
  private getVideoInfo(videoId: string): Omit<VideoData, 'transcript' | 'transcriptLength' | 'totalDuration'> {
    return {
      id: videoId,
  ...
  private searchInTranscript(
    transcript: TranscriptEntry[],
    query: string,
    contextWords: number = 10
  ): Omit<SearchResult, 'videoId' | 'videoUrl' | 'videoTitle'>[] {
    const results: Omit<SearchResult, 'videoId' | 'videoUrl' | 'videoTitle'>[] = [];
...
- VIDEO_ID (11 characters)` }],
    ...
    try {
      ...
      const transcript: TranscriptEntry[] = transcriptData.map(entry => ({
        text: entry.text,
  ...
  private async searchTranscripts(args: any): Promise<CallToolResult> {
  ...
  private async listVideos(args: any): Promise<CallToolResult> {
    if (this.videos.size === 0) {
      return {
  ...
  private async getVideoTranscript(args: any): Promise<CallToolResult> {
  ...
  private async removeVideo(args: any): Promise<CallToolResult> {
    const { videoId } = args;
  ...
  private async clearAllVideos(args: any): Promise<CallToolResult> {
